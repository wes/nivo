...
type InteractivityProps<DatumProp, ElementType> = Partial<Record<'onClick' | 'onMouseEnter' | 'onMouseLeave' | 'onMouseMove', (datum: DatumProp, event: React.MouseEvent<ElementType>) => void>>;
...
export type CalendarCanvasProps = Dimensions & CalendarData & Partial<CommonCalendarProps & InteractivityProps<Omit<Datum, 'data' | 'value'> | Datum, HTMLCanvasElement> & {
    pixelRatio: number;
}>;
...
export type TimeRangeDayProps = Record<'x' | 'y' | 'borderWidth' | 'rx' | 'ry' | 'height' | 'width', number> & {
