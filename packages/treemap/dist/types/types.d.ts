...
import { Box, Dimensions, Theme, ValueFormat, PropertyAccessor, MotionProps, SvgDefsAndFill } from '@nivo/core';
...
export type ComputedNodeWithoutStyles<Datum extends object> = Omit<ComputedNode<Datum>, 'color' | 'opacity' | 'borderColor' | 'labelTextColor' | 'parentLabelTextColor'>;
...
export type CustomCanvasLayer<Datum extends object> = (ctx: CanvasRenderingContext2D, props: CustomLayerProps<Datum>) => void;
export type NodeMouseEventHandler<Datum extends object> = (node: ComputedNode<Datum>, event: MouseEvent) => void;
...
export type TreeMapSvgProps<Datum extends object> = Partial<TreeMapCommonProps<Datum>> & TreeMapDataProps<Datum> & Dimensions & SvgDefsAndFill<ComputedNode<Datum>> & {
    nodeComponent?: NodeComponent<Datum>;
    layers?: (LayerId | CustomSvgLayer<Datum>)[];
};
export type TreeMapHtmlProps<Datum extends object> = Partial<TreeMapCommonProps<Datum>> & TreeMapDataProps<Datum> & Dimensions & {
    nodeComponent?: NodeComponent<Datum>;
    layers?: (LayerId | CustomHtmlLayer<Datum>)[];
};
export type TreeMapCanvasProps<Datum extends object> = Partial<Omit<TreeMapCommonProps<Datum>, 'enableParentLabel' | 'parentLabel' | 'parentLabelSize' | 'parentLabelPosition' | 'parentLabelPadding' | 'parentLabelTextColor' | 'onMouseEnter' | 'onMouseLeave'>> & TreeMapDataProps<Datum> & Dimensions & {
