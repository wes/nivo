...
export type PreSimulationDatum<RawDatum> = Pick<ComputedDatum<RawDatum>, 'id' | 'group' | 'value' | 'size' | 'data'> & Partial<Pick<ComputedDatum<RawDatum>, 'x' | 'y' | 'index'>>;
...
export type SwarmPlotCustomLayer<RawDatum, Scale extends ScaleLinear<number> | ScaleTime<string | Date> | ScaleOrdinal<string, number> = ScaleLinear<number>> = React.FC<SwarmPlotCustomLayerProps<RawDatum, Scale>>;
export type SwarmPlotLayer<RawDatum, Scale extends ScaleLinear<number> | ScaleTime<string | Date> | ScaleOrdinal<string, number> = ScaleLinear<number>> = SwarmPlotLayerId | SwarmPlotCustomLayer<RawDatum, Scale>;
export type SizeSpec<RawDatum> = number | ((datum: RawDatum) => number) | {
    key: string;
    values: number[];
    sizes: number[];
};
...
export type SwarmPlotCommonProps<RawDatum> = {
    ...
    layers: SwarmPlotLayer<RawDatum, ScaleLinear<number> | ScaleTime<string | Date> | ScaleOrdinal<string, number>>[];
    animate: boolean;
    motionConfig: MotionProps['motionConfig'];
    role: string;
    renderWrapper?: boolean;
};
export type SwarmPlotSvgProps<RawDatum> = SwarmPlotCommonProps<RawDatum> & MouseHandlers<RawDatum> & {
...
export type SwarmPlotCanvasProps<RawDatum> = SwarmPlotCommonProps<RawDatum> & Pick<MouseHandlers<RawDatum>, 'onMouseMove' | 'onClick'> & {
    ...
    pixelRatio: number;
    renderCircle: (ctx: CanvasRenderingContext2D, props: {
