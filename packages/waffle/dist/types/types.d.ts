...
export declare const isDataCell: <D extends Datum>(cell: Cell<D>) => cell is DataCell<D>;
...
export type MouseHandler<D extends Datum, E extends Element> = (data: ComputedDatum<D>, event: MouseEvent<E>) => void;
...
export type WaffleSvgProps<D extends Datum = Datum> = DataProps<D> & Dimensions & Partial<CommonProps<D>> & SvgDefsAndFill<ComputedDatum<D>> & Partial<MouseHandlers<D, SVGGeometryElement>> & {
...
export type WaffleHtmlProps<D extends Datum = Datum> = DataProps<D> & Dimensions & Partial<CommonProps<D>> & Partial<MouseHandlers<D, HTMLElement>> & {
    layers?: WaffleHtmlLayer<D>[];
    cellComponent?: CellComponent<D>;
    motionStagger?: number;
    testIdPrefix?: string;
};
export type CanvasProps<D extends Datum = Datum> = DataProps<D> & Dimensions & Partial<CommonProps<D>> & Partial<Omit<MouseHandlers<D, HTMLCanvasElement>, 'onMouseEnter' | 'onMouseLeave'>> & {
    legends?: LegendProps[];
    pixelRatio?: number;
};
